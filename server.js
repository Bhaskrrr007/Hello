const backendURL = "https://localhost:3000.com"; // Change this
...
function validateKey() {
    ...
    .then(data => {
        if (data.success) {
            ...
            attemptsLeft--;
            let msg = document.getElementById("attempt-message");
            msg.innerText = `Incorrect Key! ${attemptsLeft} attempts left.`;
            msg.style.color = "red";

            if (attemptsLeft <= 0) {
                msg.innerText = "Access Blocked for 24 Hours!";
                document.getElementById("activation-key").disabled = true;
            }
        }
    })
